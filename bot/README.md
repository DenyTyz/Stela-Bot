...
<h3>Stela Bot — Python Discord Bot + FastAPI Backend</h3>
...
This folder contains the Stela Discord bot built on `discord.py v2` alongside a `FastAPI` backend that powers the web dashboard. Everything runs from a single `python Stela.py` command.
...
- Stela Devs watermark on every source file
...
2. Choose **Cloudflared**, name it (e.g. `stela-api`), save
