...
from core import Cog, stela, Context
...
class Music(commands.Cog):
    ...
    async def inactivity_timer(self, guild):
        ...
        if len(guild.voice_channels[0].members) == 1:
            ...
            if player:
                ...
                try:
                    support = Button(label='Support', style=discord.ButtonStyle.link, url='https://discord.gg/steladev')
    ...
    async def play_source(self, ctx, query):
        ...
        if isinstance(tracks, wavelink.Playlist):
            ...
            await ctx.send(view=CV2(f"{ZPLUS} Added playlist [{tracks.name}](https://discord.gg/steladev) with **{len(tracks.tracks)} songs** to the queue."))
            if not vc.playing:
            ...
            track = tracks[0]
            await vc.queue.put_wait(track)
            await ctx.send(view=CV2(f"{ZPLUS}   Added [{track.title}](https://discord.gg/steladev) to the queue."))
            if not vc.playing:
    ...
    async def handle_spotify_link(self, ctx, vc, link, type_):
        try:
            if type_ == "track":
                ...
                await ctx.send(view=CV2(f"{ZPLUS} Added **{c}** of **{playlist_length}** tracks from **playlist** **[{playlist_info['name']}](https://discord.gg/steladev)** to the queue."))
                await lmao.delete()
                ...
                await ctx.send(view=CV2(f"{ZPLUS} Added all tracks from album **[{album_info['name']}](https://discord.gg/steladev)** to the queue."))
                if not vc.playing:
                    next_track = await vc.queue.get_wait()
                    await vc.play(next_track)
                    await self.display_player_embed(vc, next_track, ctx)
