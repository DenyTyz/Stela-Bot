...
from core import stela, Cog
from discord.ext import commands
import aiosqlite
from datetime import datetime, timedelta

class AutoBlacklist(Cog):
    def __init__(self, client: stela):
        self.client = client
                    ...
                    if channel:
                        embed = discord.Embed(
                            ...
                            description=(
                                ...
                                f"If you believe this is a mistake, please contact our [Support Server](https://discord.gg/steladev) with any proof if possible."
                            ),
                            color=0xFF0000
                        )
            ...
            if len(self.guild_command_tracking[guild_id]) > 8:
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        ...
                        f"If you believe this is a mistake, please contact our [Support Server](https://discord.gg/steladev)."
                    ),
                    color=0xFF0000
                )
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                if message.content in (f'<@{self.bot_user_id}>', f'<@!{self.bot_user_id}>'):
                    ...
                    embed = discord.Embed(
                        ...
                        description=f"**{message.author.mention} has been blacklisted for repeatedly mentioning me. If you believe this is a mistake, please contact our [Support Server](https://discord.gg/steladev) with any proof if possible.**",
                        color=0xFF0000
                    )
                    await message.channel.send(embed=embed)
                    return
    ...
    async def on_command(self, ctx):
        ...
        if retry:
            async with aiosqlite.connect(self.db_path) as db:
                ...
                embed = discord.Embed(
                    title=f"{ZWARNING} User Blacklisted",
                    description=f"**{ctx.author.mention} has been blacklisted for spamming commands. If you believe this is a mistake, please contact our [Support Server](https://discord.gg/steladev) with any proof if possible.**",
                    color=0xFF0000
                )
