...
async def list_guilds(bot: "stela" = Depends(get_bot)):
    """
...
async def get_guild_details(guild_id: int, bot: "stela" = Depends(get_bot)):
    """
...
async def patch_guild_j2c(guild_id: int, data: J2CUpdate, bot: "stela" = Depends(get_bot)):
    import aiosqlite
...
async def get_guild_logging(guild_id: int, bot: "stela" = Depends(get_bot)):
    """
...
async def patch_guild_logging(guild_id: int, data: LoggingUpdate, bot: "stela" = Depends(get_bot)):
    """
...
async def get_leveling_leaderboard(guild_id: int, bot: "stela" = Depends(get_bot)):
    db = await db_manager.get_connection('db/leveling.db')
    cursor = await db.execute(
...
async def get_guild_channels(guild_id: int, bot: "stela" = Depends(get_bot)):
    guild = bot.get_guild(guild_id)
    if not guild:
        raise HTTPException(status_code=404, detail="Guild not found")
...
async def get_guild_roles(guild_id: int, bot: "stela" = Depends(get_bot)):
    guild = bot.get_guild(guild_id)
    if not guild:
        raise HTTPException(status_code=404, detail="Guild not found")
